...
describe('Testing toLocalePercentFormat...', () => {
  ...
  it('Should throw exception when parameters are invalid', () => {
    ...
    expect(() => toLocalePercentFormat()).toThrow('Only numeric values are accepted!');
    expect(() => toLocalePercentFormat(NaN)).toThrow('Only numeric values are accepted!');
    expect(() => toLocalePercentFormat('a thousand')).toThrow('Only numeric values are accepted!');
  });
});
